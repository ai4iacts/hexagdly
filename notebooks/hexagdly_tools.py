...
def plot_hextensor(tensor, image_range=(0,None), channel_range=(0,None), cmap='Greys', figname='figure', mask=[]):
    ...
    try:
        ...
        pass
    inshape = np.shape(tensor[image_range[0]:image_range[1], channel_range[0]:channel_range[1]])
    ...
    for i in range(nimages):
        ...
        for x in range(np.shape(tensor[image_range[0]+a, channel_range[0]+b])[1]):
            for y in range(np.shape(tensor[image_range[0]+a, channel_range[0]+b])[0]):
                if npixel not in mask:
                    intensity = tensor[image_range[0]+a,channel_range[0]+b,y,x]
                    hexagon = RegularPolygon((x*np.sqrt(3)/2, -(y+np.mod(x,2)*0.5)), 6, 0.577349, orientation=np.pi/6)
        ...
        ax.set_ylim([-1.15*np.shape(tensor[image_range[0]+a, channel_range[0]+b])[1]-1,1])
        ax.set_axis_off()
        p = PatchCollection(np.array(hexagons[i]), cmap=cmap, alpha=0.9, edgecolors='k', linewidth=1)
